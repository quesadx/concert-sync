...
def validate_action(request: Dict[str, Any]) -> Tuple[bool, Optional[str], Optional[str]]:
    """
...
def validate_reserve_payload(request: Dict[str, Any]) -> Tuple[bool, Optional[str]]:
    ...
    if not isinstance(section_str, str):
        return False, f"RESERVE: Field 'section' must be string, got {type(section_str).__name__}"
    
    try:
        section = Section[section_str]
    except KeyError:
        valid_sections = [s.name for s in Section]
        return False, f"RESERVE: Section '{section_str}' not supported. Valid: {valid_sections}"
    ...
    if row >= max_rows or col >= max_cols:
        return False, f"RESERVE: Seat ({row}, {col}) out of bounds for {section_str} ({max_rows}x{max_cols})"
...
def validate_confirm_payload(request: Dict[str, Any]) -> Tuple[bool, Optional[str]]:
    ...
    if not isinstance(tx_id, str):
        return False, f"CONFIRM: Field 'transaction_id' must be string, got {type(tx_id).__name__}"
...
def validate_cancel_payload(request: Dict[str, Any]) -> Tuple[bool, Optional[str]]:
    ...
    tx_id = request.get("transaction_id")
    if not isinstance(tx_id, str):
        return False, f"CANCEL: Field 'transaction_id' must be string, got {type(tx_id).__name__}"
...
def validate_response(response: Dict[str, Any], expected_status: Optional[str] = None) -> Tuple[bool, Optional[str]]:
    """
